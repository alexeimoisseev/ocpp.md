...
deduplicates shared types, and outputs:
  ...
  - OCPP-2.1-Schemas-{Block}.md  (19 files)
"""
...
SCHEMAS_OUTPUT_DIR = REPO_ROOT / "docs" / "OCPP-2.1-Schemas"
DATATYPES_OUTPUT_DIR = REPO_ROOT / "docs"

# Message → functional block mapping (OCPP 2.1 A–S taxonomy, per Part 2 spec)
BLOCK_MAP = {
    "Security": [
        "SecurityEventNotification",
    ],
    "Provisioning": [
        "BootNotification", "Heartbeat", "GetBaseReport", "GetReport",
        "NotifyReport", "GetVariables", "SetVariables", "SetNetworkProfile",
        "Reset",
    ],
    "Authorization": [
    ...
    "Transactions": [
        "TransactionEvent", "GetTransactionStatus",
    ],
    "RemoteControl": [
    ...
    "Availability": [
        "ChangeAvailability", "StatusNotification",
    ],
    "Reservation": [
        "ReserveNow", "CancelReservation", "ReservationStatusUpdate",
    ],
    "TariffAndCost": [
        "CostUpdated", "GetTariffs", "SetDefaultTariff",
        "ChangeTransactionTariff", "ClearTariffs", "NotifySettlement",
        "NotifyWebPaymentStarted", "VatNumberValidation",
    ],
    "MeterValues": [
        "MeterValues",
    ],
    ...
    "Certificates": [
        ...
        "GetCertificateChainStatus", "SignCertificate", "CertificateSigned",
    ...
    "Diagnostics": [
        ...
        "GetPeriodicEventStream", "AdjustPeriodicEventStream",
    ...
    "Bidirectional": [
        "NotifyAllowedEnergyTransfer", "AFRRSignal",
    ],
    "DERControl": [
        "GetDERControl", "SetDERControl", "ClearDERControl",
        "ReportDERControl", "NotifyDERAlarm", "NotifyDERStartStop",
    ],
    "BatterySwap": [
...
# Message direction: "CS → CSMS", "CSMS → CS", or "Both" (bidirectional)
DIRECTION_MAP = {
    # Security
    ...
    "OpenPeriodicEventStream": "CS → CSMS",
    "ClosePeriodicEventStream": "CS → CSMS",
    "GetPeriodicEventStream": "CSMS → CS",
    "AdjustPeriodicEventStream": "CSMS → CS",
    "NotifyPeriodicEventStream": "CS → CSMS (one-way SEND)",
    # Display
...
# Functional block letter codes (OCPP 2.1 A–S taxonomy)
BLOCK_CODES = {
    "Security": "A", "Provisioning": "B", "Authorization": "C",
    "LocalAuthList": "D", "Transactions": "E", "RemoteControl": "F",
    "Availability": "G", "Reservation": "H", "TariffAndCost": "I",
    "MeterValues": "J", "SmartCharging": "K", "Firmware": "L",
    "Certificates": "M", "Diagnostics": "N", "Display": "O",
    "DataTransfer": "P", "Bidirectional": "Q", "DERControl": "R",
    "BatterySwap": "S",
}
...
def build_message_registry(schemas):
    ...
    # Sweep for standalone schemas (neither *Request nor *Response):
    # OCPP 2.1 introduces one-way SEND messages such as
    # NotifyPeriodicEventStream. Store them with a request payload and no
    # response, matching the dict shape used elsewhere.
    for filename, schema in schemas.items():
...
def generate_block_md(block_name, messages_in_block, message_registry, shared_types, local_types, type_usage_map):
    ...
    for msg_name in messages_in_block:
        ...
        if resp is None:
            # One-way SEND message (e.g. NotifyPeriodicEventStream): the
            # request is rendered above; there is no response to document.
            lines.append("*One-way message — no response.*")
            lines.append("")
        elif resp:
...
def main():
    ...
    # Generate DataTypes.md (under docs/)
    print("Generating OCPP-2.1-DataTypes.md...")
    DATATYPES_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    # Generate block files (in OCPP-2.1-Schemas/ subdirectory)
    SCHEMAS_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    for block_name, messages_in_block in BLOCK_MAP.items():
        print(f"Generating OCPP-2.1-Schemas-{block_name}.md...")
    ...
    print(f"\nDone! Generated {1 + len(BLOCK_MAP)} files:")
    print(f"  - OCPP-2.1-DataTypes.md ({enum_count} enums, {composite_count} composite types)")
    for block_name, msgs in BLOCK_MAP.items():
        print(f"  - OCPP-2.1-Schemas-{block_name}.md ({len(msgs)} messages)")
